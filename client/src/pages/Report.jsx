...
export default function Report() {
    ...
    return (
        <div className='bg-gray-100 min-h-screen flex flex-col  items-center p-6'>
            ...
            <div className="sm:p-2 md:p-6 bg-[#F0F7FF] shadow-md rounded-md lg:h-[530px]">
                    ...
                    <div className="md:overflow-y-scroll w-full lg:h-[300px] mt-4">
                        {students.map((std, index) => (
                            <div key={index} className="grid grid-cols-5 h-16 gap-x-4 p-6">
                                ...
                                <span className='col-span-2 md:hidden sm:text-sm md:text-lg text-center'>{std.name.split(' ').map((char, index) => (
                                    <p key={index}>{char}</p>
                                ))}</span>
                                <span className='col-span-2 hidden md:block sm:text-sm md:text-lg text-center'>{std.name}</span>
